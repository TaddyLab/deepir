...
model.  This makes for easy implementation in any system that has already been
engineered to fit such language representations.  
%This will lead to faster deployment and lower development costs.  
The strategy is also very interpretable: whatever intuition one has about the
distributed language model can be applied directly to the resulting
inversion-based classification rule.  Finally, given its relatively strong
performance in classification, inversion can be used to add a plausible model
for reader understanding and inference on to a given language composition
model.
...
popular \texttt{gensim} python library \cite{rehurek_software_2010}; the extended
library as well as code to reproduce all of the results in this
paper are available on \texttt{github}.
